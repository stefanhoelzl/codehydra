...
**Changing the root.** The app:start `migrations` hook compares `paths.workspaces` with the root in use and asks on the starting screen. **Migrate** (empty folder only) moves managed clones and nothing else: worktrees are adopted in place (`external` tag) so agents keep their conversations and editors their state, and new worktrees go to the new root. The switch (`paths.workspaces-current`) is the commit point: failures before it are undone (copies deleted, `git worktree repair` pointed back, tags removed); after it they are reported. Path-keyed state (`hooks.trusted`, `auto-workspaces`, screenshots) is moved by each owner (`moveProjects`). **Use as is** switches without moving. The new folder is recorded with symlinks and junctions resolved, because git reports worktree paths resolved and ownership is a path-prefix test. See `src/modules/workspaces-root/migrate.ts`.
...
- **One file per entry, git-style.** No subdirectories, no `10-`/`20-` ordering: a repository that wants several steps writes them in one script. The extension never gates (the shebang decides), which is what lets a `.py` hook be ordinary.
- **Platform suffixes, and no tiebreak.** `<entry>.win|linux|mac[.*]` pins a file to one platform: it beats the unsuffixed file there and never runs elsewhere, so a repository can ship a shebang script beside a `.win.cmd` (cmd.exe cannot run a bare file). Two files left for one platform used to resolve to the first by name, which let a stale `x.bak` silently replace `x.sh`; every tiebreak picks the wrong file for someone, so ambiguity is now an error notification naming the files and counts as a hook failure (the deletion gate fails closed). It is judged after the trust question, so a project answered Never is not held up by it. Directories and symlinks are ignored before choosing, so they cannot shadow a real file.
- **Cancel, no timeout.** A timeout long enough for `pnpm install` is too long to be an escape, and one short enough to be an escape kills real setup work. Instead, while a blocking hook's process runs the hooks module registers it with the presenter (`trackRunningHook`), which offers Cancel where the user is already looking: the startup loading screen, the active workspace's loading panel, the deletion panel (through `cancelRunningHooks`), or — for a hook no surface shows, after a 1.5 s grace so quick hooks never flash — a sidebar notification. Cancel aborts the run's `AbortSignal`; the runner kills the process tree and fails the run as `<entry> was canceled`, so each entry's ordinary failure rule applies unchanged. The tree kill has to reach grandchildren (`sh` → script → its commands): one holding the inherited pipes keeps the spawn from finishing, so the Unix kill walks descendants with `pgrep -P` and remembers them across the SIGTERM and SIGKILL passes. `app:shutdown` → `stop` aborts every run still in flight (event entries too) and waits for the kills, and a run started after it is canceled before it spawns: a hook is a child of the app, the OS does not take it down with its parent, and one left sitting in its worktree keeps that directory from being removed on Windows.
- **Reported, not silently skipped, when not executable.** Unlike git: a hook the author wrote and that never runs is the hardest failure to notice. The 126/127 messages are hedged ("a file could not be executed", "a command was not found — the shebang interpreter, or one the script ran") because a script that runs a missing or non-executable command exits with the same codes.
- **stderr at `warn`.** It is the only record of a hook that never reached an editor (a deletion gate, a failed open), and a line a script chose to print is worth keeping at the default log level. The output sink holds lines for an editor that has not connected yet, and holds none for one that is gone for good — a workspace being deleted — so nothing buffers for an IDE that will never flush it.
...
- **`before-worktree-deleted` fails closed.** A refusal (`{"blocked":true}`, exit 0) and a broken hook (non-zero exit) both stop the deletion, but are reported differently: one is a policy decision, the other a hook failure. Dismiss force-deletes and skips hooks entirely, as the escape from a gate that refuses wrongly. Cancel on the panel is the escape from one that hangs.
