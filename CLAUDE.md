...
| `experimental.busy-during-background-shell` | `false`   | Keep workspace busy while background shells run. `true` = every background shell; array of regexes (config.json only) = only shells whose command matches, so dev servers don't pin busy |
