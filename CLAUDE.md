...
| Repository hooks   | Scripts a repository ships under `.codehydra/` in its **worktree**, run at curated moments. `hooks/<name>` blocks and exchanges JSON (stdin in, stdout out); `events/<name>` is fire-and-forget with output ignored — the directory _is_ the blocking rule. One file per entry, git-style: if it exists it runs, any extension, launched through the platform shell so the shebang and the exec bit decide. v1: `hooks/after-worktree-created` (replaces `.keepfiles`; may return `env`/`title`/`tags`), `hooks/before-worktree-deleted` (may refuse with `{"blocked":true,"reason":…}`; owns a deletion-progress row), `events/on-workspace-created`. Trust is asked at first fire per project (Always/Once/Skip/Never; Always/Never persist in `state.json`). See docs/HOOKS.md                                                                                                                                                                                                                                                                                                                                                        |
