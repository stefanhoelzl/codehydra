...
| Repository hooks   | Scripts a repository ships under `.codehydra/` in its **worktree**, run at curated moments. `hooks/<name>` blocks and exchanges JSON (stdin in, stdout out); `events/<name>` is fire-and-forget with output ignored — the directory _is_ the blocking rule. One file per entry, git-style: if it exists it runs, launched through the platform shell so the shebang and the exec bit decide. The filename is `<name>` or `<name>.<anything>` — the extension never gates (a `.py` hook is ordinary, and Windows _needs_ `.cmd` since cmd cannot run a bare file); two files claiming one entry warns and takes the first by name. v1: `hooks/after-worktree-created` (replaces `.keepfiles`; may return `env`/`title`/`tags`), `hooks/before-worktree-deleted` (may refuse with `{"blocked":true,"reason":…}`; owns a deletion-progress row), `events/on-workspace-created`. Trust is asked at first fire per project (Always/Once/Skip/Never; Always/Never persist in `state.json`). See docs/HOOKS.md                                                                                                                                  |
