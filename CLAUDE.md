...
| `ide-server`                                | `code-server` | Embedded IDE server distribution: `code-server`\|`vscodium`. `vscodium` downloads the official VSCodium reh-web build (Open VSX, first-class Windows) instead of code-server                                                                                                                                                   |
| `code-server.port`                          | (auto)        | code-server port (auto = 25448 in prod, branch-derived in dev). Used when `ide-server=code-server`                                                                                                                                                                                                                             |
| `vscodium.port`                             | (auto)        | VSCodium port (auto = 25449 in prod, branch-derived in dev). Distinct from code-server so each distribution keeps its own IndexedDB origin. Used when `ide-server=vscodium`                                                                                                                                                    |
| `version.code-server`                       | (built-in)    | code-server version override                                                                                                                                                                                                                                                                                                   |
...
| `experimental.busy-during-background-shell` | `true`        | Keep workspace busy while background shells run. `true` = every background shell; `false` = never; array of regexes (config.json only) = only shells whose command matches, so dev servers don't pin busy                                                                                                                      |
...
| `experimental.github.template`              | `null`        | Inline Liquid template for GitHub auto-workspaces; set to enable. Edited inline in settings (multiline text control with a help panel). Kept out of bug reports (`omit`) but shown in the clear in settings. Legacy `experimental.github.template-path` (a file path) is deprecated and migrated into this key on startup.     |
...
| `experimental.youtrack.template`            | `null`        | Inline Liquid template for YouTrack auto-workspaces; set to enable. Edited inline in settings (multiline text control with a help panel). Kept out of bug reports (`omit`) but shown in the clear in settings. Legacy `experimental.youtrack.template-path` (a file path) is deprecated and migrated into this key on startup. |
