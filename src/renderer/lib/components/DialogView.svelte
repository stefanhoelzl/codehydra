<!--
  ...
  Generic declarative dialog renderer.
  Renders any DialogConfig as: faded logo backdrop + centered card with sections + actions.
  Sections are rendered by type: text, progress, selection, table.
  Actions are rendered as vscode-button elements in a footer row.
-->
<script lang="ts">
  import type {
    DialogConfig,
  ...
  onMount(() => {
    setTimeout(() => {
      const selected = cardRef?.querySelector("[aria-checked='true']") as HTMLElement | null;
      selected?.focus();
    }, 0);
  });
