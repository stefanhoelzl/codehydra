...
describe("registerLogHandlers", () => {
  let mockLogging: ReturnType<typeof createMockLogging>;
  let handlers: Map<string, (event: unknown, payload: unknown) => void>;

  beforeEach(() => {
    vi.clearAllMocks();
    mockLogging = createMockLogging();
    handlers = new Map();
    ...
    registerLogHandlers(mockLogging);
  });
  ...
  describe("LOG_DEBUG handler", () => {
    it("creates logger and calls debug", () => {
      ...
      expect(mockLogging.createLogger).toHaveBeenCalledWith("ui");
      const logger = mockLogging.getLogger("ui");
  ...
  describe("logger name validation", () => {
    ...
    it("accepts valid api logger name", () => {
      ...
      expect(mockLogging.createLogger).toHaveBeenCalledWith("api");
    });
  ...
  describe("error handling", () => {
    it("swallows errors and does not throw", () => {
      ...
      mockLogging.createLogger.mockImplementation(() => {
