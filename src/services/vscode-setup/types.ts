...
/**
 * Configuration for a bundled extension in manifest.json.
 */
...
export function validateExtensionsConfig(value: unknown): ExtensionsConfigValidationResult {
  if (typeof value !== "object" || value === null) {
    return { isValid: false, error: "manifest.json must be an object" };
  }
  ...
  if (!Array.isArray(obj.marketplace)) {
    return { isValid: false, error: "manifest.json must have a 'marketplace' array" };
  }
  ...
  if (!Array.isArray(obj.bundled)) {
    return { isValid: false, error: "manifest.json must have a 'bundled' array" };
  }
